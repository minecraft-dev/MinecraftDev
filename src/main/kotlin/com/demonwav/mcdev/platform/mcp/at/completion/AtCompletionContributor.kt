            ...
            srgResult.addElement(PrioritizedLookupElement.withPriority(
                LookupElementBuilder
                    ...
                    .withInsertHandler handler@ { context, _ ->
                        ...
                        currentElement.replace(AtElementFactory.createFieldName(context.project, memberReference.name))
                ...
                LookupElementBuilder.create(method.nameAndParameterTypes)
                    ...
                    .withInsertHandler handler@ { context, _ ->
                        ...
                        if (currentElement is AtFieldName) {
                            ...
                            val children = parent.node.getChildren(TokenSet.create(AtTypes.OPEN_PAREN, AtTypes.CLOSE_PAREN))
                            if (children.size == 2) {
                                parent.node.removeRange(children[0], children[1].treeNext)
                            }
                        }

                        currentElement.replace(AtElementFactory.createFunction(project, memberReference.name + memberReference.descriptor))

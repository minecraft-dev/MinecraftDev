...
import com.demonwav.mcdev.platform.mixin.util.AccessorAddMethodFix
...
class GenerateAccessorAction : NonMixinCodeInsightAction() {
    ...
    override fun invoke(project: Project, editor: Editor, file: PsiFile) {
        ...
        candidates.addAll(
            targetClass.fields
                .filter { it.modifierList?.hasExplicitModifier(PsiModifier.PUBLIC) != true }
                .map { PsiFieldMember(it) }
        )
        candidates.addAll(
            targetClass.methods
                .filter { !it.modifierList.hasExplicitModifier(PsiModifier.PUBLIC) }
    ...
    private fun generateAccessor(
        ...
        if (generateGetter) {
            ...
            AccessorAddMethodFix(method, mixin).applyFix()
        }
        if (generateSetter) {
            val method = factory.createMethodFromText(
                """

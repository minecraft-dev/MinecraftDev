/*
 * Minecraft Dev for IntelliJ
 *
 * https://minecraftdev.org
 *
 * Copyright (c) 2017 minecraft-dev
 *
 * MIT License
 */

package com.demonwav.mcdev.i18n

object I18nConstants {
    const val FILE_EXTENSION = "lang"
    const val DEFAULT_LOCALE = "en_us"
    const val I18N_CLIENT_CLASS = "net.minecraft.client.resources.I18n"
    const val I18N_COMMON_CLASS = "net.minecraft.util.text.translation.I18n"
    const val TRANSLATION_COMPONENT_CONSTRUCTOR = "TextComponentTranslation"
    const val TRANSLATION_COMPONENT_CLASS = "net.minecraft.util.text.$TRANSLATION_COMPONENT_CONSTRUCTOR"
    const val COMMAND_EXCEPTION_CONSTRUCTOR = "CommandException"
    const val COMMAND_EXCEPTION_CLASS = "net.minecraft.command.$COMMAND_EXCEPTION_CONSTRUCTOR"
    const val FORMAT = "func_135052_a"
    const val TRANSLATE_TO_LOCAL = "func_74838_a"
    const val TRANSLATE_TO_LOCAL_FORMATTED = "func_74837_a"
    const val SET_BLOCK_NAME = "func_149663_c"
    const val SET_ITEM_NAME = "func_77655_b"
}

...
class AwInspectionSuppressor : InspectionSuppressor {

    override fun isSuppressedFor(element: PsiElement, toolId: String): Boolean {
        val entry = element.parentOfType<AwEntry>(withSelf = true)
        ...
        val file = element.containingFile as AwFile
    ...
    override fun getSuppressActions(
        ...
        val entry = element as? AwEntry
            ?: element.parentOfType<AwEntry>(withSelf = true)
            ?: PsiTreeUtil.getPrevSiblingOfType(element, AwEntry::class.java) // For when we are at a CRLF
        return if (entry != null) {
            arrayOf(AwSuppressQuickFix(entry, toolId), AwSuppressQuickFix(element.containingFile, toolId))
        } else {
            arrayOf(AwSuppressQuickFix(element.containingFile, toolId))
        }
    }

    class AwSuppressQuickFix(element: PsiElement, val toolId: String) :
        LocalQuickFixOnPsiElement(element), SuppressQuickFix {

        override fun getText(): @IntentionName String = when (startElement) {
            is AwEntry -> "Suppress $toolId for entry"
            is AwFile -> "Suppress $toolId for file"
        ...
        override fun invoke(
            ...
            when (startElement) {
                is AwEntry -> suppressForEntry(startElement)
                is AwFile -> suppressForFile(startElement)
            }
        }

        private fun suppressForEntry(entry: AwEntry) {
        ...
        private fun suppressForFile(file: AwFile) {
            ...
            val newCommentText =
                commentText.substring(suppressStart, suppressEnd) + ",$toolId" + commentText.substring(suppressEnd)
            val newComment = AwElementFactory.createComment(file.project, newCommentText)

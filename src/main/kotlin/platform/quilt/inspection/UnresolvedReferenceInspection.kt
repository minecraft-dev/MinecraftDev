...
package com.demonwav.mcdev.platform.quilt.inspection
...
class UnresolvedReferenceInspection : LocalInspectionTool() {
    ...
    override fun getStaticDescription() = "Reports unresolved references in Quilt mod JSON files."

    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        if (holder.file.name == QuiltConstants.QUILT_MOD_JSON) {
    ...
    override fun processFile(file: PsiFile, manager: InspectionManager): List<ProblemDescriptor> {
        if (file.name == QuiltConstants.QUILT_MOD_JSON) {

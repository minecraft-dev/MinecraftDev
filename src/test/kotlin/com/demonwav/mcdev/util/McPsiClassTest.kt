...
class McPsiClassTest : OuterClassTest() {
    ...
    fun `test outer full qualified name`() =
        assertEquals("com.example.test.OuterClass", outerClass.fullQualifiedName)
    fun `test outer short name`() = assertEquals("OuterClass", outerClass.shortName)

    fun `test outer anonymous full qualified name`() =
        assertEquals("com.example.test.OuterClass$1", outerAnonymousClass.fullQualifiedName)
    fun `test outer anonymous short name`() = assertEquals("OuterClass.1", outerAnonymousClass.shortName)

    fun `test inner full qualified name`() =
        assertEquals("com.example.test.OuterClass\$InnerClass", innerClass.fullQualifiedName)
    ...
    fun `test inner anonymous short name`() =
        assertEquals("OuterClass.InnerClass.1", innerAnonymousClass.shortName)
    ...
    fun `test inner anonymous inner short name`() =
        assertEquals("OuterClass.InnerClass.1.AnonymousInnerClass", innerAnonymousInnerClass.shortName)
    ...
    fun `test self referencing generic`() =
        assertEquals(
